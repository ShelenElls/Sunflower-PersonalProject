...
class Profile(models.Model):
    ...
    account = models.ForeignKey(User, related_name="profiles", on_delete=models.PROTECT)        
...
class Income(models.Model):
    ...
    type_disperse = [('SALARY', 'Salary'), ('HOURLY', 'Hourly'), ('FLUCTUATING', 'fluctuating')]
    ...
    dispersement_type = models.CharField(max_length=20, choices=type_disperse, default="HOURLY", null=True)
...
# is now available in the views 
    ...
    def pay_hourly_calc(self):
        ...
        if self.income_type['ACTIVE'] == True:
            if self.type_disperse['HOURLY'] == True:
    ...
    def pay_calc_salary(self):
        pay_total_monthly = 0
        if self.income_type['ACTIVE'] == True:
            if self.type_disperse['SALARY'] == True:
